...
class Exp3(Helper):
    ...
    def update(self, model: nn.Module, train: Dataset, sample: Dataset):
        ...
        max_samples = math.floor(self.train.x.shape[0] * POSITIVE_RATIO)
        num_top = int(0.2 * max_samples)  # 60% highest scores
        num_random = max_samples - num_top  # 40% randomly chosen
        sorted_indices = positive_indices.cpu().numpy()[np.argsort(-y_prob_all[positive_indices].cpu().numpy())]
        # Select the top 60% highest scores
        selected_top = sorted_indices[:num_top].flatten()  # Ensure it's 1D

        # Ensure remaining_indices is a 1D NumPy array
        remaining_indices = np.array(sorted_indices[num_top:]).flatten()

        # Randomly select the remaining 40% from the rest
        if num_random > 0 and len(remaining_indices) > 0:
            selected_random = np.random.choice(remaining_indices, size=num_random, replace=False).flatten()
        else:
            selected_random = np.array([])  # Edge case: No available indices

        # Ensure both arrays are 1D before concatenation
        sampled_indices = np.concatenate([selected_top, selected_random]).flatten()

        # Find additional indices where 0.5 < y_prob_all ≤ 0.6
        # extra_indices = pt.where((y_prob_all > 0.5) & (y_prob_all <= 0.55))[0]
