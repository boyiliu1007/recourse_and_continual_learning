...
for i in range(10):
    ...
    recoursed_data = recourse(model, selected_subset, 400)
    train.x[selected_indices] = recoursed_data.x.detach()
    train.y[selected_indices] = recoursed_data.y.detach()

    # test recourse function
    if not test_only_selected_data_modified(model, before_recoursed, train, selected_indices):
        print("test error")

    model = training(model, train, 200, loss_list)
    # print(model.linear.weight.data) # debug log
