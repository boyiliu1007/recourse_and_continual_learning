...
def continual_training(si: SynapticIntelligence, dataset: Dataset, max_epochs: int, loss_list = None, lambda_ = 0.5):
    ...
    for _ in range(max_epochs):
        ...
        for X_batch, Y_batch in train_loader:
            ...
            # Original loss
            primary_loss = criterion(outputs, Y_batch)
            
            # SI regularization loss
            si_loss = si.compute_si_loss(lambda_)
            
            # balanced prediction constraint
            # Count predictions > 0.5 (sigmoid activation)
            positive_preds = pt.sum(outputs > 0.5)
            target_positives = X_batch.size(0) / 2  # Half the batch size
            balance_loss = (target_positives - positive_preds) ** 2
            
            # Combine losses (you may want to add a scaling factor for balance_loss)
            balance_lambda = 0.001  # Hyperparameter to control the strength of the balance constraint
            loss = primary_loss + si_loss + balance_lambda * balance_loss
            # print("primary_loss: ", primary_loss)
            # print("si_loss: ", si_loss)
            # print("balance_loss: ", balance_lambda * balance_loss)
