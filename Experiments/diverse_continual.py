...
class Example9_Continual_Learning(Helper):
    ...
    def update(self, model: nn.Module, train: Dataset, sample: Dataset):
        ...
        # diverse k
        tmp_train = deepcopy(self.train)
        positive_mask = (y_prob_all > 0.5).nonzero(as_tuple=True)[0]
        negative_mask = (y_prob_all <= 0.5).nonzero(as_tuple=True)[0]
        chosen_mask = self.findDiverse(self.train.x[positive_mask], self.train.x.shape[0] // 2)
        print(len(chosen_mask))
        result_mask = positive_mask[chosen_mask]
        result_ftr_mask = positive_mask[~pt.isin(positive_mask, result_mask)]
        new_mask = pt.cat((result_mask, negative_mask))
        new_mask = new_mask.sort()[0]
        self.train.x = self.train.x[new_mask]
        self.train.y = self.train.y[new_mask]
        tmp_train.y[result_ftr_mask].zero_()
        ...
        self.train.x = pt.cat((tmp_train.x[result_ftr_mask], self.train.x))
        self.train.y = pt.cat((pt.zeros_like(tmp_train.y[result_ftr_mask]), self.train.y))
